...
import com.collect.vo.CoworkersVO;
...
public class ReportServiceTest {
    ...
    public void testLookReports() {
        ...
        Task task = new Task(user0.getId(), "软工三作业检查", 10, 10, 0, 0L, "a", "b", "dfji");
        taskMapper.insert(task);
        ...
        assert reportService.lookReports(task.getId()).getData().size() == 2;
        ReportVO reportVO = (ReportVO) reportService.lookReports(task.getId()).getData().get(0);
        assert reportVO.getPaths().size() == 3;
        reportVO = (ReportVO) reportService.lookReports(task.getId()).getData().get(1);
    ...
    @Test
    public void testLookReportsNotFound() {
        assert reportService.lookReports(2318290).getCode() == 4000;
    }
    ...
    public void testGetCommentsUnderReport() {
        ...
        ResponseVO comments = reportService.getCommentsUnderReport(report.getId());
    ...
    public void testGiveMarkAndComment() {
        ...
        // 在报告下面进行评论
        Comment comment1 = new Comment(null, report.getId(), worker1.getId(), 5.0, "这里有一个问题", null, null);
        Report newStar = (Report) reportService.giveMarkAndComment(comment1.toDTO()).getData().get(0).toDTO().toPO();
        assert newStar.getStar() == 5.0;
        assert newStar.getStarNum() == 1;
        Comment comment2 = new Comment(null, report.getId(), worker2.getId(), 4.0, "这里有两个问题", null, null);
        Report newStar2 = (Report) reportService.giveMarkAndComment(comment2.toDTO()).getData().get(0).toDTO().toPO();
        ...
        Report newStar3 = (Report) reportService.giveMarkAndComment(comment3.toDTO()).getData().get(0).toDTO().toPO();
    ...
    @Test
    public void testFindSimilarReports() {
        // 注册为发包方
        User user = new User(null, "czw@qq.com", "123456", null, UserIdentity.DELIVER, "czw",null,null);
        userMapper.insert(user);
        long time = System.currentTimeMillis() * 2;
        ...
        List<VO> similarReports = reportService.findTargetReports(similarStrategy,report.getId()).getData();
    ...
    public void testGiveAnnotation() {
        ...
        reportService.giveAnnotation(report.toDTO(), worker2.getId());
    ...
    public void testShowAnnotation() {
        ...
        report.setNote("/ddiwojdoiwjdi%hdsiadioho%/dsdjsidojdioajdio%djiadjio%/");
        reportService.giveAnnotation(report.toDTO(), worker2.getId());
        report.setNote("/ddiwojdoiwjdi%hdsiadioho%/dsdjsidojdioajdio%djiadji%/");
        reportService.giveAnnotation(report.toDTO(), worker3.getId());
        assert reportService.showAnnotation(task.getId(), worker1.getId()).getData().size() == 1;
        assert ((ReportVO) reportService.showAnnotation(task.getId(), worker1.getId()).getData().get(0)).getNote().equals("/ddiwojdoiwjdi%hdsiadioho%/dsdjsidojdioajdio%djiadji%/");
        //assert reportMapper.selectById(report.getId()).getNote().equals("/ddiwojdoiwjdi%hdsiadioho%/dsdjsidojdioajdio%djiadjio%/");
    ...
    public void testFindCoworkers() {
        ...
        report.setNote("/ddiwojdoiwjdi%hdsiadioho%/dsdjsidojdioajdio%djiadjio%/");
        reportService.giveAnnotation(report.toDTO(), worker2.getId());
        Report report2 = new Report(null, worker1.getId(), task.getId(), "写得真好啊", "1.2.3.4.", "ios", 0.0, 0);
        reportMapper.insert(report2);
        report2.setNote("/ddiwojdoiwjdi%hdsiadioho%/dsdjsidojdioajdio%djiadji%/");
        reportService.giveAnnotation(report2.toDTO(), worker2.getId());
        assert ((CoworkersVO) reportService.findCoworkers(task.getId(), worker1.getId()).getData().get(0)).getCoworkers().size() == 1;
        assert Objects.equals(((CoworkersVO) reportService.findCoworkers(task.getId(), worker1.getId()).getData().get(0)).getCoworkers().get(0).getId(), worker2.getId());
        reportService.giveAnnotation(report2.toDTO(), worker3.getId());
        assert ((CoworkersVO) reportService.findCoworkers(task.getId(), worker1.getId()).getData().get(0)).getCoworkers().size() == 2;
